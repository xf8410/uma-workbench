...
class SidHealthProbe(private val clock:()->Long=System::currentTimeMillis){
 suspend fun probe(session:GameSession,request:suspend(GameRequest)->GameResponse):SidHealthResult{
  val local=diagnoseLocally(session);if(local!=null)return local
  val healthRequest=GameRequest(GameEndpoint.LOAD_INDEX,session.sid,session.viewerId,ProtocolRequestTemplates.forEndpoint(GameEndpoint.LOAD_INDEX,session),headers=buildHeaders(session),timestamp=clock())
  val response=request(healthRequest);val interpreted=ProtocolResponseInterpreter.interpret(response.statusCode,response.bodyDecrypted?:response.body)
 ...
 fun diagnoseLocally(session:GameSession):SidHealthResult?{
  ...
  val(explanation,action)=when(diagnosis){SidDiagnosis.MISSING->"当前会话没有 SID。" to "先通过 hlpatch Dump、登录链或手动输入取得完整 SID。";SidDiagnosis.ANONYMOUS->"SID 对应 viewer_id=0，只能用于 boot。" to "执行安卓登录链，将 SID 与有效 viewer_id 绑定。";SidDiagnosis.UNBOUND->"SID 尚未与有效 viewer_id 绑定。" to "执行 login → start_session → load/index。";SidDiagnosis.EXPIRED->"会话已被标记为过期。" to "重新获取账号 token 并执行登录链。";else->error("Unexpected local diagnosis: $diagnosis")}
  return SidHealthResult(session,diagnosis,clock(),null,null,explanation,action,null)
 }
 private fun buildHeaders(session:GameSession):Map<String,String> = buildMap{put("SID",session.sid);put("ViewerID",session.viewerId.toString());put("APP-VER",session.appVer);session.resVer?.let{put("RES-VER",it)};put("Device","4");put("Device-Sub-Type","1");put("User-Agent","UnityPlayer/2022.3.62f2");session.deviceId?.let{put("Device-Id",it)};session.accountToken?.let{put("Authorization",it)}}
}

object LoginChainPlanner{
 fun next(result:SidHealthResult):GameEndpoint?=when(result.diagnosis){SidDiagnosis.ACTIVE->null;SidDiagnosis.ANONYMOUS,SidDiagnosis.UNBOUND,SidDiagnosis.EXPIRED,SidDiagnosis.MISSING,SidDiagnosis.UNKNOWN->GameEndpoint.LOGIN}
