...
class SidHealthProbe(private val clock: () -> Long = System::currentTimeMillis) {
    suspend fun probe(
        ...
        val interpreted = ProtocolResponseInterpreter.interpret(
            response.statusCode,
            response.bodyDecrypted ?: response.body
        )
        ...
        return SidHealthResult(
            ...
            clock(),
            response.statusCode,
            interpreted.protocolCode,
            interpreted.diagnosis.explanation,
            interpreted.diagnosis.suggestedAction,
            response
        )
    }
...
object LoginChainPlanner {
    fun next(result: SidHealthResult): GameEndpoint? = when (result.diagnosis) {
        ...
        SidDiagnosis.ANONYMOUS,
        SidDiagnosis.UNBOUND,
        SidDiagnosis.EXPIRED,
        SidDiagnosis.MISSING,
        SidDiagnosis.UNKNOWN -> GameEndpoint.LOGIN
    }
    ...
    fun remainingFrom(endpoint: GameEndpoint): List<GameEndpoint> = when (endpoint) {
        ...
        GameEndpoint.BOOT,
        GameEndpoint.PRE_SIGNUP,
        GameEndpoint.SIGNUP,
        GameEndpoint.UNKNOWN -> emptyList()
    }
}
