...
object ArchiveIndexer {
    ...
    fun readTarBatch(
        ...
        val input = BufferedInputStream(openInput())
        ...
        input.use { tar ->
            while (true) {
                ...
                if (index < resumeAt) {
                    tar.skipFully(size + paddingFor(size))
                    index++
                    continue
                }
                if (output.size == maxEntries) {
                ...
                output += ArchiveEntryEntity(
                    ...
                    path = path,
                    directory = directory,
                    uncompressedBytes = if (directory) 0 else size,
                    compressedBytes = null,
                    crc32 = null,
                    unsafePath = ArchiveAnalyzer.isUnsafePath(path),
                    modifiedAt = parseTarOctal(header, 136, 12).takeIf { it > 0 }?.times(1000),
                    type = "TAR:${if (typeFlag.code == 0) '0' else typeFlag}"
                )
    ...
    private fun InputStream.skipFully(byteCount: Long) {
        ...
        while (remaining > 0) {
            ...
            if (skipped > 0) remaining -= skipped else {
                ...
                require(count >= 0) { "Archive entry data is truncated" }
                remaining -= count
            }
        }
    }
