...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val projects = _currentWorkspaceId.flatMapLatest { id -> if (id == null) flowOf(emptyList<ProjectEntity>()) else workspaceManager.observeProjects(id) }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())
    val recentFiles = _currentWorkspaceId.flatMapLatest { id -> if (id == null) flowOf(emptyList<RecentFileEntity>()) else workspaceManager.observeRecentFiles(id) }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())
    val openTabs = _currentWorkspaceId.flatMapLatest { id -> if (id == null) flowOf(emptyList<OpenTabEntity>()) else db.openTabs().observe(id) }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())
    ...
    val messages = _currentConversationId.flatMapLatest { id -> if (id == null) flowOf(emptyList<MessageEntity>()) else repository.messages(id) }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())
    ...
    fun openFile(uri: String, name: String) = viewModelScope.launch {
        ...
        db.openTabs().upsert(OpenTabEntity(tabId, wsId, uri, name, openTabs.value.size))
        _activeTabId.value = tabId
        runCatching {
            app.contentResolver.openFileDescriptor(Uri.parse(uri), "r")?.use { fd ->
                _fileContent.value = android.os.ParcelFileDescriptor.AutoCloseInputStream(fd).use { String(it.readBytes(), Charsets.UTF_8) }
            }
    ...
    fun importFile(uris: List<Uri> = emptyList()) = viewModelScope.launch { uris.forEach { openFile(it.toString(), it.lastPathSegment ?: it.toString()) } }

    fun newConversation() = viewModelScope.launch {
        val now = System.currentTimeMillis(); val conv = ConversationEntity(UUID.randomUUID().toString(), "新对话", now, now)
        repository.createConversation(conv); _currentConversationId.value = conv.id
    }
    fun sendAgentMessage(text: String) = viewModelScope.launch {
        ...
        repository.addMessage(MessageEntity(UUID.randomUUID().toString(), convId, null, null, repository.nextMessageSequence(convId), "user", text, "COMPLETE", now))
        repository.addMessage(MessageEntity(UUID.randomUUID().toString(), convId, null, null, repository.nextMessageSequence(convId), "assistant", "已收到：$text\n\n（AI Provider 尚未接入，当前为占位回复）", "COMPLETE", System.currentTimeMillis()))
    }
    ...
    private fun extractJsonField(json: String, field: String): String? {
        for (pattern in listOf("\"$field\"", "'$field'", "$field:")) {
            val idx = json.indexOf(pattern, ignoreCase = true)
            if (idx >= 0) { val after = json.substring(idx + pattern.length).trimStart(' ', ':', '"', '\''); val end = after.indexOfFirst { it == '"' || it == '\'' || it == ',' || it == '}' || it == '\n' }; val value = if (end >= 0) after.substring(0, end) else after; if (value.isNotEmpty()) return value }
        }
        return null
    }
}
