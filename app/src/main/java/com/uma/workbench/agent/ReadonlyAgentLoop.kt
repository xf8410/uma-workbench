...
class ReadonlyAgentLoop(private val provider: AiStreamingProvider, private val executor: ReadonlyAgentToolExecutor, private val limits: ReadonlyAgentLoopLimits = ReadonlyAgentLoopLimits()) {
    suspend fun run(initial: AiGenerationRequest, observer: ReadonlyAgentObserver? = null): ReadonlyAgentRunResult {
        ...
        repeat(limits.maxModelRounds) { roundIndex ->
            ...
            val round = ReadonlyAgentRound(roundIndex + 1, text.toString(), calls, outcomes, resolvedModel, roundUsage)
            rounds += round
            ...
            if (calls.isEmpty()) {
                ...
                return ReadonlyAgentRunResult(initial.requestId, text.toString(), rounds, aggregateUsage, resolvedModel, messages.toList())
            }
            ...
            calls.zip(outcomes).forEach { (call, outcome) ->
                messages += AiPromptMessage("tool", outcome.toModelContent(), toolCallId = call.id, toolName = call.name)
            }
        }
        error("Agent 循环异常结束")
    }
