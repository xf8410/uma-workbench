...
data class SubAgentLimits(val maxTasksPerDispatch:Int=4,val maxParallelTasks:Int=2,val maxDepth:Int=1,val maxInstructionCharacters:Int=16_384,val maxCombinedAnswerCharacters:Int=131_072,val timeoutMillisPerTask:Long=120_000){init{require(maxTasksPerDispatch in 1..16);require(maxParallelTasks in 1..8);require(maxParallelTasks<=maxTasksPerDispatch);require(maxDepth in 1..4);require(maxInstructionCharacters in 1..100_000);require(maxCombinedAnswerCharacters in 1..1_000_000);require(timeoutMillisPerTask in 1_000..600_000)}}
...
/** Provider-independent orchestration. Children have isolated context and an individual wall-clock limit. */
class SubAgentCoordinator(private val loopFactory:SubAgentLoopFactory,private val limits:SubAgentLimits=SubAgentLimits(),private val requestIdFactory:()->String={UUID.randomUUID().toString()}){
 suspend fun dispatch(parentRequest:AiGenerationRequest,tasks:List<SubAgentTask>,depth:Int=1):List<SubAgentOutcome>{
  require(depth in 1..limits.maxDepth){"子 Agent 深度 $depth 超过上限 ${limits.maxDepth}"};require(tasks.isNotEmpty()){"子 Agent 任务不能为空"};require(tasks.size<=limits.maxTasksPerDispatch){"子 Agent 任务 ${tasks.size} 个，超过上限 ${limits.maxTasksPerDispatch}"};require(tasks.map{it.id}.distinct().size==tasks.size){"子 Agent taskId 必须唯一"};tasks.forEach{require(it.id.isNotBlank()){"子 Agent taskId 不能为空"};require(it.instruction.isNotBlank()){"子 Agent ${it.id} 指令不能为空"};require(it.instruction.length+it.evidenceRequirements.length<=limits.maxInstructionCharacters){"子 Agent ${it.id} 指令超过字符上限 ${limits.maxInstructionCharacters}"}}
 ...
 private suspend fun execute(parent:AiGenerationRequest,task:SubAgentTask,depth:Int):SubAgentOutcome{
  ...
  return try{val run=withTimeout(limits.timeoutMillisPerTask){loopFactory.create().run(parent.copy(requestId=requestId,messages=messages,tools=ReadonlyAgentToolSchemas.childReadOnly))};SubAgentOutcome.Success(SubAgentResult(task.id,requestId,run.completeAnswer,run.rounds,run.usage,run.model))}catch(t:TimeoutCancellationException){SubAgentOutcome.Failure(SubAgentFailure(task.id,"子 Agent ${task.id} 超过运行时限 ${limits.timeoutMillisPerTask}ms"))}catch(c:CancellationException){throw c}catch(e:Throwable){SubAgentOutcome.Failure(SubAgentFailure(task.id,e.stackTraceToString()))}
 }
