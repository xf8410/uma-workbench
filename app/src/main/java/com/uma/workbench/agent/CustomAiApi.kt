                ...
                if (message.toolCalls.isNotEmpty()) put("tool_calls", buildJsonArray {
                    message.toolCalls.forEach { call -> add(buildJsonObject {
                        ...
                        put("function", buildJsonObject {
                            put("name", call.name)
                            put("arguments", runCatching { json.parseToJsonElement(call.completeArgumentsJson) }.getOrElse { JsonPrimitive(call.completeArgumentsJson) })
                        })
                    }) }
                }
            }) }
        }
        return protocol.requestTemplate.replace("{{modelJson}}", JsonPrimitive(request.model ?: configuredModel).toString()).replace("{{messagesJson}}", messages.toString()).replace("{{requestIdJson}}", JsonPrimitive(request.requestId).toString())
    }
