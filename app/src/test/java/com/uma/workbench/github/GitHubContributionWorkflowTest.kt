...
class GitHubContributionWorkflowTest {
    ...
    private class FakeGateway : GitHubContributionGateway {
        val calls = mutableListOf<String>()
        override suspend fun fork(owner: String, repo: String, confirmationId: String?) = GitHubForkResult(binding, "https://github.com/me/project", true).also { calls += "fork" }
    ...
    @Test fun requiresOrderedStages() = runTest {
        val fake = FakeGateway(); val workflow = GitHubContributionWorkflow(fake)
    ...
    @Test fun cannotCreatePrBeforeCommit() = runTest {
        ...
        assertThrows(IllegalArgumentException::class.java) { runTest { workflow.createPullRequest(progress, "title", "body", false, "confirm") } }
    }
}
